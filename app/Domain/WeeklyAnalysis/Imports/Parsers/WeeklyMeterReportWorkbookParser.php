...
class WeeklyMeterReportWorkbookParser extends AbstractWorkbookParser
{
    public function type(): WorkbookType
    {
        return WorkbookType::WeeklyMeterReport;
    }
    ...
    protected function parseSpreadsheet(Spreadsheet $spreadsheet): ParsedWorkbook
    {
        ...
        foreach ($spreadsheet->getWorksheetIterator() as $sheet) {
            ...
            if (! str_contains((string) $this->cell($sheet, 'B', 1), 'Handmeter/RHP Shipments')) {
                $issues[] = new WorkbookValidationIssue('Expected Handmeter/RHP Shipments title in B1.', $sheetName, 1);
            }
            ...
            for ($row = 6; $row <= min($sheet->getHighestDataRow(), 42); $row++) {
                ...
                $rows[] = [
                    ...
                    'forecast_units' => $this->cell($sheet, 'N', $row),
                    'forecast_variance' => $this->cell($sheet, 'O', $row),
                ];
            }
        }
